...
if (!is.null(params$dataframe_csv) && file.exists(params$dataframe_csv)) {
  df <- read.csv(params$dataframe_csv, stringsAsFactors = FALSE)
...
cat(paste0("<b>GC path</b>: ", params$gc_path, "  \n"))
cat(paste0("<b>HTML report</b>: ", params$report_html, "\n"))
```
...
gc_expected <- list()
if (!is.null(params$gc_path) && file.exists(params$gc_path)) {
  suppressPackageStartupMessages(library(jsonlite))
  gc_expected <- jsonlite::fromJSON(params$gc_path)
}
...
get_core_sample_name <- function(filename) {
  ...
  base <- sub("_fastqc\\\\.html$", "", base, ignore.case = TRUE)
  base
}
...
get_read_direction_from_file <- function(file_path) {
  ...
  if (grepl("read1", lower) || grepl("r1", file_path)) {
    return("R1")
  }
  if (grepl("read2", lower) || grepl("r2", file_path)) {
    return("R2")
  }
  NA
}
...
sample_names <- unique(vapply(html_files, get_core_sample_name, character(1)))
...
df_qc <- data.frame(matrix(NA, nrow = length(sample_names), ncol = length(cols)),
...
for (file_path in html_files) {
  ...
  if (is.na(read_dir) || !(sample %in% sample_names)) {
    next
  }
...
if (length(sample_names) > 0) {
  for (sample in sample_names) {
    r1_total <- df_qc[sample, "Total Sequences R1"]
    r2_total <- df_qc[sample, "Total Sequences R2"]
    r1_poor <- df_qc[sample, "Sequences flagged as poor quality R1"]
    r2_poor <- df_qc[sample, "Sequences flagged as poor quality R2"]
    r1_gc <- df_qc[sample, "%GC R1"]
    r2_gc <- df_qc[sample, "%GC R2"]
...
df_local <- NULL
if (!is.null(params$dataframe_csv) && file.exists(params$dataframe_csv)) {
  df_local <- read.csv(params$dataframe_csv, stringsAsFactors = FALSE)
...
df_mlst <- NULL
if (!is.null(params$dataframe_csv) && file.exists(params$dataframe_csv)) {
  df_mlst <- read.csv(params$dataframe_csv, stringsAsFactors = FALSE, check.names = FALSE)
}
...
if (is.null(df_mlst) || !all(c("SAMPLE", "SUBTYPE", "SPECIES") %in% names(df_mlst))) {
  ...
  if (nrow(mlst_df) == 0) {
    ...
    table_html <- paste0(
      "<table class=\"annotation-table mlst-table\">",
      "<thead><tr>",
      "<th>Species</th>",
      "<th>Subtype</th>",
      "<th>Sample</th>",
...
build_gene_table <- function(df_subset, table_class) {
...
df_annotations <- NULL
if (!is.null(params$dataframe_csv) && file.exists(params$dataframe_csv)) {
  df_annotations <- read.csv(params$dataframe_csv, stringsAsFactors = FALSE, check.names = FALSE)
}
...
if (is.null(df_annotations) || !"PREDICTION_SOURCE" %in% names(df_annotations)) {
  ...
  if (nrow(vir_df) == 0) {
    ...
    cat(build_gene_table(vir_df, "annotation-table vfdb-table"))
  }
}
```
...
if (is.null(df_annotations) || !"PREDICTION_SOURCE" %in% names(df_annotations)) {
  ...
  if (nrow(res_df) == 0) {
    ...
    cat(build_gene_table(res_df, "annotation-table resistome-table"))
  }
}
```
...
if (is.null(df_annotations) || !"PREDICTION_SOURCE" %in% names(df_annotations)) {
  ...
  if (nrow(pla_df) == 0) {
    ...
    cat(build_gene_table(pla_df, "annotation-table plasmid-table"))
  }
}
```
    ...
    for (sp_dir in species_dirs) {
      ...
      p <- ggplot(df_long, aes(x = GeneIndex, y = SampleIndex, fill = Presence)) +
        ...
        theme(
          ...
          axis.text.y = element_text(size = 6, hjust = 1, color = "#4b5563"),
      ...
      if (file.exists(plot_path)) {
        ...
        cat(sprintf(
          '<div style="width:100%%;aspect-ratio:20/1;margin:0 auto;"><img src="%s" alt="Pangenome heatmap" style="width:100%%;height:100%%;display:block;object-fit:contain;background:#ffffff;" /></div>\n\n',
          img_uri
        ))
      } else {
